...
func NewRepository(Type *string, storagePath *string) *Repository {
	var rType string
	var rPath string
	if Type == nil { rType = "file" } else { rType = *Type }
	if storagePath == nil { rPath = defaultStoragePath} else { rPath = *storagePath}
	return &Repository{Type: rType, StoragePath: rPath}
}

func (r *Repository) Find(id string) *User {
	switch r.Type {
		case "file": return r.findFile(id)
		default: log.Fatal("[FATAL] Non existing type of repository set!")
	}
	return nil
}

func (r *Repository) findFile(id string) *User {
	file, err := os.OpenFile(r.StoragePath + id + ".json", os.O_RDONLY, 0666); if err != nil {
...
func (r *Repository) saveFile(u *User)  {
	...
	var file *os.File
	file, err = os.OpenFile(r.StoragePath + u.Id + ".json", os.O_RDWR|os.O_TRUNC, 0666); if err != nil {
		file, err = os.Create(r.StoragePath + u.Id + ".json"); if err != nil {
